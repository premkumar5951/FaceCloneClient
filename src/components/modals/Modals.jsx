  ...
  return (
    <div>
      <div onClick={handleClickOpen} className='divmain'>
        {edit ? (
          <div className='divedit'>Edit</div>
        ) : (
          <div className='divedit'>Delete</div>
        )}
      </div>
      <Dialog className='dialog' open={open} onClose={handleClose}>
        {edit ? (
          <>
            <DialogTitle className='dialogtitle'>
              {postupdated ? (
                ...
                <>
                  Edit Post <EditIcon className='mx-1' />
                </>
              )}
            </DialogTitle>
            <DialogContent>
              <form className='sharewrapper mt-2'>
                <div className='sharetop'>
                  ...
                  <textarea
                    ...
                    rows='1'
                    name='sharetextedit'
                    readonly={postupdated ? "readonly" : null}
                    value={updatepost}
                    onChange={(e) => {
            ...
            <DialogActions>
              {postupdated ? (
                <Button onClick={handleClose} className='canclebtn text-danger'>
                  OK
                </Button>
              ) : (
                <>
          ...
          <>
            <DialogTitle className='dialogtitle'>
              Delete <DeleteIcon className='mx-1' />
            </DialogTitle>
            <DialogContent>
              <DialogContentText className='deletetext'>
                {postdeleted ? (
                  <>
                    <WarningIcon htmlColor='tomato' className='mx-1 warn' />
                    Post deleted
                  </>
                ) : (
                  <>
            ...
            <DialogActions>
              {postdeleted ? null : (
                <>
                  <Button
                    onClick={handleClose}
                    className='canclebtn text-danger'>
                    Cancle
                  </Button>
                  <Button
                    className='deletebtn text-success'
