...
useEffect(()=>{
  try{
    (async()=>{
      const res=await fetch(`${process.env.REACT_APP_HOST}/users?userId=${post.userId}`,{
        method:'GET',
        headers:{
   ...
   const like=async(e)=>{
     ...
     if(e.detail===1){
      const res=await fetch(`${process.env.REACT_APP_HOST}/posts/${post._id}/like`,{
        method:'PUT',
        headers:{
          "Content-type":"application/json"
        },
...
setsameuser(true)
  ...
  return (
    ...
    <img src={postuser.profilepic?`${postuser.profilepic}`:`${PFPERSON}`} alt="" className="postimg"/>
...
/></span>
       ...
       {sameuser?<LongMenu post={post} username={username} profileUser={profileUser} setprofileposts={setprofileposts}/>:null} 
        ...
        <div className="mainmgdiv">
          <img src={post.img?`${post.img}`:null} alt="" className="mainimg"/>
        </div>
